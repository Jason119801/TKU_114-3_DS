...
public class Q12_InventoryCatalog {
    ...
    public boolean addProduct(Q12_Product product) {
        ...
        for (Q12_Product p : products) {
            if (p.getId().equalsIgnoreCase(product.getId())) return false;
        }
    ...
    private void merge(
        ...
        while (i <= mid && j <= right) {
            if (temp[i].getId().compareTo(temp[j].getId()) <= 0) {
                data[k++] = temp[i++];
            } else {
                data[k++] = temp[j++];
            }
    ...
    public Q12_Product binarySearchById(
        ...
        while (left <= right) {
            ...
            int cmp = sortedProducts[catalogTraceD19A].getId().compareToIgnoreCase(searchId);
            
            if (cmp == 0) return sortedProducts[catalogTraceD19A];
            if (cmp < 0) left = catalogTraceD19A + 1;
            else right = catalogTraceD19A - 1;
        }
        return null;
    }
